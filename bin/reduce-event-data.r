...
###
## Time-stamp: <2018-05-21 14:11:35 gepr>
##
...
for (prefix in prefixes) {
  ...
  for (expDir in argv) {
    ...
    write.csv(x=ExpStatEvent, file=paste(expDir, "_stat_exp_",prefix,".csv", sep=""), row.names=TRUE)
    write.csv(x=TrialStatEvent, file=paste(expDir, "_stat_MCtrials_",prefix,".csv", sep=""), row.names=FALSE)
